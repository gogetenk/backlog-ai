...
public class ApiService : IApiService
{
    ...
    public async Task<TResponse> PostPromptAsync<TResponse>(string prompt)
    {
        try
        {
            var response = await _http.PostAsJsonAsync("YourApiEndpoint", new { Prompt = prompt });
    ...
    public async Task<List<BacklogItem>> GetBacklogItemsAsync()
    {
        try
        {
    ...
    public record BacklogItem(string Title, string ShortDescription, string DetailedDescription);
